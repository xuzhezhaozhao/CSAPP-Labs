/*
 * 采用书上的implicit list 方法实现, 每个block有header和footer, 
 * init, malloc, free都是用的书上的代码, realloc方法的实现是
 * 先检测与原块相邻的块是否为空且空间与原块相加能否满足要求,能
 * 满足就直接分配, 否则用malloc分配合适大小的空间, 并free原块地址
 * 空间.
 */
#include <stdio.h>
#include <stdlib.h>
