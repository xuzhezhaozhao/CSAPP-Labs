...
Each phase has three variants: "a", "b", and "c". Each student gets a
bomb with a randomly chosen variant for each phase.  In addition, most
phase variants are parameterized by randomly chosen constants that are
assigned when a particular bomb is constructed.  Thus, each student
gets a unique bomb that they must solve themselves.  The unique
...
*****************************
Q. Is there any extra credit for solving the secret phase?
A. What secret phase? :-)
